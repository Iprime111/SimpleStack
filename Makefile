...
debug: export buildDir_ = $(outDir)/debug
...
release: export CXXFLAGS = $(libFlags) -std=c++17 -Wno-write-strings -O3
release: export buildDir_ = $(outDir)/release
...
srcObjects = $(addprefix $(buildDir_)/, $(sources:.cpp=.o))

.PHONY: all prepare build_docs

all: $(buildDir_)/$(target)

# TODO -M -MD -MMD -MF ...

$(srcObjects): $(buildDir_)/%.o: $(srcDir)/%.cpp
	@echo [CXX] -c $< -o $@
	@$(CXX) $(CXXFLAGS) -c $< -o $@


$(buildDir_)/$(target): prepare $(srcObjects)
